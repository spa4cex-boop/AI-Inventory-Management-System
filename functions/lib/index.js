...
function getUserRole(user) {
    if (!user)
        return 'staff';
    return user.role || user.role_claims?.role || 'staff';
}
    ...
    try {
        const response = await axios_1.default.post(OPENROUTER_URL, {
            ...
            messages: [
                {
                    role: 'system',
                    content: 'You are an inventory assistant for a SaaS inventory management platform. Provide stock recommendations, order guidance, and product insights. Use plain text responses. Do not expose internal debug details.',
                },
